...
//            Log.i(AppConfig.TAG, "TProxyStartService...")
    ...
    private fun buildConfig(): String {
        ...
        return buildString {
            ...
            appendLine("  udp: 'tcp'")
            val su = LocalSocksAuth.username()
            val sp = LocalSocksAuth.password()
            if (su.isNotEmpty() && sp.isNotEmpty()) {
                appendLine("  username: '${su.replace("'", "''")}'")
                appendLine("  password: '${sp.replace("'", "''")}'")
            }
